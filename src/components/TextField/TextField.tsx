...
const TextField = forwardRef<HTMLDivElement, TextFieldProps>((props, ref) => {
  ...
  const disabledClass = useMemo(
    () => (disabled ? 'bg-gray-100 opacity-40 cursor-not-allowed' : ''),
    [disabled]
  );
  ...
  return (
    <div
      ...
      onClick={onClick}
      onKeyDown={onKeyDown}
      className={`relative box-border inline-flex border text-black ${withPrefixClass} ${withSuffixClass} ${
        size ? TextFieldSize[size] : ''
      } ${disabledClass} ${errorClass} ${fullWidthClass}`}
      style={style}
    >
      {prefix && (
        <div className="flex items-center flex-shrink-0 left-1 text-2xl px-1">
          {prefix}
        </div>
      )}
      {children}
      {suffix && (
        <div className="flex items-center flex-shrink-0 right-1 text-2xl px-1">
          {suffix}
        </div>
      )}
    </div>
